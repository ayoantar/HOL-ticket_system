-- Fix function parameter types to match PostgreSQL ENUM types
-- The request_type column is an ENUM, so we need to handle type casting properly

-- Drop existing functions and recreate with proper type handling
DROP TRIGGER IF EXISTS set_request_number_trigger ON requests CASCADE;
DROP FUNCTION IF EXISTS set_request_number(request_type_enum, character varying) CASCADE;
DROP FUNCTION IF EXISTS generate_request_number(text, text) CASCADE;

-- Create department-based request number generation function with proper types
