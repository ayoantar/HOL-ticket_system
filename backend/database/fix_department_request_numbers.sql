-- Fix request number generation to use department-based formatting
-- This will generate numbers like REQ-EVT-001, REQ-GFX-001, etc.

-- Drop existing functions and recreate with department-based logic
DROP TRIGGER IF EXISTS set_request_number_trigger ON requests CASCADE;
DROP FUNCTION IF EXISTS set_request_number() CASCADE;
DROP FUNCTION IF EXISTS generate_request_number() CASCADE;

-- Create department-based request number generation function
...
BEGIN
    IF NEW.request_number IS NULL THEN
        NEW.request_number := generate_request_number(NEW.request_type, NEW.department);
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
